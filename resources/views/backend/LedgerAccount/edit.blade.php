...
<form action="{{ $form['url'] }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="row">
        <div class="offset-lg-3 col-lg-6">
            @if($model->type == LedgerAccount::TYPE_SIMPLE)
            <div class="form-group mb-3">
                <h5>Current Balance # <small class="text-muted"> {{ $model->current_balance}} </small></h5>
                <x-Inputs.text-field name="name" label="Account Name" placeholder="Enter Account Name" :value="$model->name" />
            </div>
            <div class="form-group mb-3">
            ...
            @elseif($model->type == LedgerAccount::TYPE_PARTY)
            <div class="form-group mb-3">
                <h5>Party # <small class="text-muted"> {{ $model->party?->name}} </small></h5>
                <h5>Opening Balance # <small class="text-muted"> {{ $model->opening_balance}} </small>,
                    <span class="fs-6">
                        To Edit this Goto Party Form
                        <a target="_blank" href="{{ route('parties.edit', [$model->party_id]) }}">
                            Click Here
                        </a>
                        </h6>
                </h5>
...
<script>
    ...
    $(document).ready(function() {
        if (type != "<?= LedgerAccount::TYPE_SIMPLE ?>") {
            $(".bank_inputs").show();
        }
    });
</script>
